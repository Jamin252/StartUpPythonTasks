...
class Invader(pygame.sprite.Sprite):
    ...
    def checkSpeed(self,level):
        self.speed = 2**(level - 1)
    def update(self, game):
        self.checkSpeed(game.level)
        oldx = self.rect.x
...
class Game():
    ...
    def startScreen(self):
        ...
        tempTxt = font.render(f"Level {self.level}", True, WHITE)
        self.screen.fill(BLACK)
        self.screen.blit(tempTxt,(SIZE[0]/2-50, SIZE[1]/2-50))
        pygame.display.flip()
        ...
        while timeDiff < 4000:
            ...
            self.screen.fill(BLACK)
            self.screen.blit(timeTxt,(SIZE[0]/2-50, SIZE[1]/2-50))
            pygame.display.flip()
            timeDiff = pygame.time.get_ticks() - tempTime
    ...
    def innitialize(self):
        ...
        self.invader_changeDirection = False
        self.forceExit = False
        self.scoreBoard.innitialize()
        for item in self.invader_group:
            item.innitialize(self)
        self.generate(NUMOFINVADERS*self.level//2 - len(self.invader_group))
        for item in self.player_group:
            item.innitialize(self.level)
        for item in self.bullet_group:
            item.kill()
